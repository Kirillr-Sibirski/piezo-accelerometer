...
% Constants
Cc = 380 * 10^(-12); % Shield to cable capacitance
Cp = 700 * 10^(-12); % Sensor capacitance w/ cable
Vcc = 5; % V

% Variables
Cf = (600)*10^(-12); % Lower freq, determines output voltage
...
d33 = 300 * 10^(-12); % piezoelectric constant calculated at 100Hz, for some reason not linear at all but ffs idk why
m = 0.1; % proof mass, kg
max_expected_a = 2*9.81; % 5g 
...
c_total = Cc+Cp+(15*10^(-12)); % 15nF is the probe capacitance
acc = 19.6; % m/s^2
v_out = 265*10^(-3); % V_peak
d33_rev = -(v_out*c_total)/(acc*m);
